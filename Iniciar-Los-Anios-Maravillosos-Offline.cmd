...
powershell.exe -NoProfile -ExecutionPolicy Bypass -Command "$limit=(Get-Date).AddSeconds(15); do { try { $response=Invoke-WebRequest -UseBasicParsing -Uri '%APP_URL%' -TimeoutSec 2; if ($response.StatusCode -eq 200) { exit 0 } } catch {}; Start-Sleep -Milliseconds 250 } while ((Get-Date) -lt $limit); exit 1" >nul 2>&1
if errorlevel 1 (
  echo No fue posible iniciar Los Anios Maravillosos Offline.
...
if defined BROWSER_EXE (
  start "Los Anios Maravillosos" "%BROWSER_EXE%" --app="%APP_URL%"
) else (
  start "" "%APP_URL%"
)
endlocal
